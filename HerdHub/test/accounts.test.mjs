...
async function login(email, password) {
  ...
  assert.equal(r.status, 200, `Login failed for ${email}: ${JSON.stringify(body)}`);
...
before(async () => {
  ...
  assert.equal(r.status, 201, `Failed to create test admin: ${r.status} ${JSON.stringify(created)}`);
...
test('1 - register happy path: 201, auto-login, role=user', async () => {
  ...
  let id;
  try {
    const r = await post('/api/register', { email: em, password: 'TestPass1!' });
    const body = await r.json();
    assert.equal(r.status, 201);
...
test('2 - register duplicate email → 409', async () => {
  ...
  try {
    ...
    const r1 = await post('/api/register', { email: em, password: 'TestPass1!' });
    assert.equal(r1.status, 201);
    const r2 = await post('/api/register', { email: em, password: 'TestPass1!' });
...
test('4 - register invalid email format → 400', async () => {
  try {
    ...
    assert.equal(status, 400, `Expected 400 but server accepted malformed email: ${JSON.stringify(body)}`);
    assert.ok(body.error);
  } finally {
...
test('5 - login correct credentials → 200 + cookie', async () => {
  ...
  let id;
  try {
    const rReg = await post('/api/register', { email: em, password: 'TestPass1!' });
...
test('7 - login unknown email → 401', async () => {
  const r = await post('/api/login', { email: 'nobody@test.invalid', password: 'TestPass1!' });
  assert.equal(r.status, 401);
});
...
test('14 - guest cannot POST /api/upload-image → 401', async () => {
  const r = await post('/api/upload-image', { name: 'x', breedId: 1, dataUrl: 'data:image/jpeg;base64,AA==', context: 'master' });
  assert.equal(r.status, 401);
});
...
test('18 - user can PUT /api/myherd', async () => {
  ...
  try {
    ...
    const accs = await (await get('/api/accounts', tAdminCk)).json();
    id = accs.find(a => a.email === em)?.id;
    const r = await put('/api/myherd', [{ id: 999, name: 'TestEntry', tags: [], imageUrl: null }], ck);
    assert.equal(r.status, 200);
    const body = await r.json();
    assert.ok(body.ok);
...
test('19 - user can POST /api/upload-image (own images)', async () => {
  ...
  try {
    ...
    const r = await post('/api/upload-image', { name: 'UploadTest', breedId, dataUrl, context: 'user' }, ck);
    assert.equal(r.status, 200);
    const body = await r.json();
    assert.ok(body.path?.startsWith('/images/'), `expected /images/ path, got: ${body.path}`);
  } finally {
...
test('23 - user cannot DELETE /api/breeds/:id → 403', async () => {
  ...
  try {
    ...
    assert.ok(breeds.some(b => b.id === breedId), 'breed should still exist after blocked delete');
  } finally {
    if (breedId) await adminDeleteBreed(breedId);
    if (id) await adminDeleteAccount(id);
  }
...
test('24 - user cannot POST /api/accounts → 403', async () => {
  ...
  try {
    ...
    const accs = await (await get('/api/accounts', tAdminCk)).json();
    id = accs.find(a => a.email === em)?.id;
    const r = await post('/api/accounts', { email: email('injected'), password: 'TestPass1!', role: 'admin' }, ck);
    assert.equal(r.status, 403);
  } finally {
    if (id) await adminDeleteAccount(id);
  }
...
test('26 - user cannot DELETE /api/accounts/:id → 403', async () => {
  ...
  try {
    ...
    const accs2 = await (await get('/api/accounts', tAdminCk)).json();
    assert.ok(accs2.some(a => a.id === victimId), 'victim account should still exist');
  } finally {
    if (victimId) await adminDeleteAccount(victimId);
    if (id) await adminDeleteAccount(id);
  }
...
test('28 - admin GET /api/accounts: list without passwordHash', async () => {
  ...
  for (const acc of body) {
    assert.ok(!('passwordHash' in acc), `passwordHash must not be exposed on ${acc.email}`);
    assert.ok('id' in acc);
...
test('29 - admin POST /api/accounts creates user account', async () => {
  ...
  let id;
  try {
    const r = await post('/api/accounts', { email: em, password: 'TestPass1!', role: 'user' }, tAdminCk);
    assert.equal(r.status, 201);
    const body = await r.json();
    assert.equal(body.email, em);
    assert.equal(body.role, 'user');
...
test('30 - admin POST /api/accounts creates admin account', async () => {
  ...
  let id;
  try {
    const r = await post('/api/accounts', { email: em, password: 'TestPass1!', role: 'admin' }, tAdminCk);
    assert.equal(r.status, 201);
    const body = await r.json();
...
test('31 - admin cannot create account with invalid role → 400', async () => {
  const r = await post('/api/accounts', { email: email('bad_role'), password: 'TestPass1!', role: 'superuser' }, tAdminCk);
...
test('33 - admin PATCH /api/accounts/:id changes password', async () => {
  ...
  try {
    ...
    const r = await patch(`/api/accounts/${id}`, { password: 'NewPass2!' }, tAdminCk);
    assert.equal(r.status, 200);
...
test('34 - admin PATCH short password → 400', async () => {
  ...
  try {
    ...
    const r = await patch(`/api/accounts/${id}`, { password: 'short' }, tAdminCk);
    assert.equal(r.status, 400);
  } finally {
    if (id) await adminDeleteAccount(id);
  }
...
test('36 - admin DELETE /api/accounts/:id removes account', async () => {
  ...
  const accs = await (await get('/api/accounts', tAdminCk)).json();
  assert.ok(!accs.some(a => a.id === id), 'deleted account must not appear in list');
  // login should fail
  const loginRes = await post('/api/login', { email: em, password: 'TestPass1!' });
...
test('39 - admin can PATCH /api/breeds/:id', async () => {
  ...
  try {
    ...
    const r = await patch(`/api/breeds/${id}`, { origin: 'TestOrigin' }, tAdminCk);
    assert.equal(r.status, 200);
    const body = await r.json();
...
test('40 - admin can DELETE /api/breeds/:id', async () => {
  ...
  assert.equal(r.status, 200);
  const breeds = await (await get('/api/breeds')).json();
  assert.ok(!breeds.some(b => b.id === id), 'breed should be gone after admin delete');
});
...
test('41 - admin can impersonate a user, session acts as that user', async () => {
  ...
  try {
    ...
    assert.equal(accsRes.status, 403, 'impersonated user session must not access admin routes');
  } finally {
...
test('42 - admin cannot start a second impersonation while already impersonating → 400', async () => {
  ...
  let tempAdminCk;
  try {
    tempAdminId   = await adminCreateAccount(tempAdminEmail,   'TestPass1!', 'admin');
    adminTargetId = await adminCreateAccount(adminTargetEmail, 'TestPass1!', 'admin');
    userTargetId  = await adminCreateAccount(userTargetEmail,  'TestPass1!', 'user');
    ...
    assert.equal(r2.status, 400, `Expected 400 (already impersonating), got ${r2.status}`);
    const body = await r2.json();
    assert.ok(body.error);
  } finally {
...
test('44 - non-admin cannot POST /api/impersonate → 403', async () => {
  ...
  try {
    ...
    const accs = await (await get('/api/accounts', tAdminCk)).json();
    id = accs.find(a => a.email === em)?.id;
    targetId = await adminCreateAccount(email('imp_target_nonadmin'), 'TestPass1!', 'user');
    const r = await post(`/api/impersonate/${targetId}`, {}, ck);
    assert.equal(r.status, 403);
  } finally {
    if (targetId) await adminDeleteAccount(targetId);
...
test('46 - admin can GET /api/admin/backup → 200 + tar.gz stream', async () => {
  ...
  assert.ok(buffer.byteLength > 0, 'Backup stream should not be empty');
});
