...
function nextName() {
  return `TestBreed_${testRunId}_${++testIdx}`;
}
...
async function registerAndLoginUser(email, password = "TestPass123!") {
  ...
  const accs = await (
    await fetch(`${baseUrl}/api/accounts`, { headers: { Cookie: testAdminCk } })
  ).json();
...
async function createTestBreed(name = nextName()) {
  ...
  assert.equal(
    r.status,
    201,
    `Failed to create test breed "${name}": ${r.status}`,
  );
...
async function assertExternalUrlFlow(savedImageUrl, label) {
  ...
  assert.equal(
    imgRes.status,
    200,
    `[${label}] local image file not served at ${savedImageUrl}`,
  );
  ...
  assert.equal(
    thumbRes.status,
    200,
    `[${label}] thumbnail not available for ${relpath}`,
  );
  assert.equal(
    thumbRes.headers.get("content-type"),
    "image/webp",
    `[${label}] thumbnail should be webp`,
  );
}
...
test(
  ...
  async (t) => {
    ...
    assert.equal(
      createRes.status,
      201,
      `Failed to create test admin: ${createRes.status} ${createBody}`,
    );
    ...
    await t.test("upload returns path with ?t= cache-buster", async () => {
      ...
      try {
        ...
        assert.ok(
          ...
          `path should start with /images/, got: ${imgPath}`,
        );
        assert.ok(
          imgPath.includes("?t="),
          `path should contain ?t= cache-buster, got: ${imgPath}`,
        );
      } finally {
    ...
    await t.test("thumbnail is served (webp) after upload", async () => {
      ...
      try {
        ...
        assert.equal(
          thumbRes.status,
          200,
          `Thumb not available: ${thumbRes.status}`,
        );
        assert.equal(
          thumbRes.headers.get("content-type"),
          "image/webp",
          "Thumb should be webp",
        );
      } finally {
        await deleteBreed(breed.id);
      }
      ...
      try {
        ...
        for (let i = 0; i < 20; i++) {
          ...
          if (
            ...
            thumb1 = Buffer.from(await res.arrayBuffer());
            break;
          }
          await new Promise((r) => setTimeout(r, 300));
        }
        ...
        assert.ok(
          p2.includes("?t="),
          "Second upload should return cache-buster",
        );
        let thumb2;
        for (let i = 0; i < 20; i++) {
          const res = await fetch(`${baseUrl}/api/thumb/${filename}`);
        ...
        assert.ok(
          thumb2,
          "Thumbnail should change after re-upload with different image",
        );
      } finally {
        await deleteBreed(breed.id);
      }
    ...
    // ── 2. ?t= stripping ───────────────────────────────────────────────────────
    await t.test(
      "PATCH breed strips ?t= from imageUrl before saving",
      async () => {
        const breed = await createTestBreed();
        try {
          const upR = await apiPost(
            `${baseUrl}/api/upload-image`,
            {
              name: breed.name,
              breedId: breed.id,
          ...
          assert.ok(
            !saved.imageUrl.includes("?"),
            `Saved imageUrl should not contain ?t=, got: ${saved.imageUrl}`,
          );
        } finally {
          await deleteBreed(breed.id);
        }
    ...
    await t.test(
      "PUT myherd strips ?t= from all imageUrls before saving",
      async () => {
        const { ck: userCk, id: userId } = await registerAndLoginUser(
          `testuser_${Date.now()}@test.invalid`,
        );
        try {
          const fakeHerd = [
            {
              id: 99901,
              name: "MyBreed",
              imageUrl: "/images/99_mybreed_1.jpg?t=1234567890",
            },
          ];
    ...
    await t.test(
      "10 - POST /api/breeds (Add New Breed): external imageUrl is downloaded and saved locally",
      async () => {
        ...
        assert.equal(
          resp.status,
          201,
          `POST /api/breeds returned ${resp.status}`,
        );
        ...
        try {
          ...
          assert.ok(
            inGrid,
            `Breed id=${breed.id} should appear in GET /api/breeds`,
          );
          assert.ok(
    ...
    await t.test(
      "11 - PATCH /api/breeds/:id (Edit master list): replacing an existing image with a new external URL downloads the new image",
      async () => {
        ...
        try {
          ...
          assert.equal(
            resp1.status,
            200,
            `Round-1 PATCH returned ${resp1.status}`,
          );
          const breed1 = await resp1.json();
          await assertExternalUrlFlow(breed1.imageUrl, "PATCH round 1");
          const content1 = Buffer.from(
            await (await fetch(`${baseUrl}${breed1.imageUrl}`)).arrayBuffer(),
          );
          ...
          assert.equal(
            resp2.status,
            200,
            `Round-2 PATCH returned ${resp2.status}`,
          );
          const breed2 = await resp2.json();
          await assertExternalUrlFlow(breed2.imageUrl, "PATCH round 2");
          const content2 = Buffer.from(
            await (await fetch(`${baseUrl}${breed2.imageUrl}`)).arrayBuffer(),
          );

          assert.notDeepEqual(
            content1,
            content2,
            "Image file must be replaced when a new external URL is set on an existing breed",
          );
          ...
          assert.ok(
            inGrid,
            `Breed id=${target.id} should appear in GET /api/breeds`,
          );
          assert.ok(
            inGrid.imageUrl?.startsWith("/images/"),
            `Grid entry imageUrl should be local, got: ${inGrid.imageUrl}`,
          );
        } finally {
    ...
    await t.test(
      "12 - PUT /api/myherd (Edit My Herd): replacing an existing image with a new external URL downloads the new image",
      async () => {
        const target = await createTestBreed();
        const seedB = await createTestBreed();
        const { ck: userCk, id: userId } = await registerAndLoginUser(
        ...
        try {
          ...
          const content1 = Buffer.from(
            await (await fetch(`${baseUrl}${entry1.imageUrl}`)).arrayBuffer(),
          );
          ...
          const content2 = Buffer.from(
            await (await fetch(`${baseUrl}${entry2.imageUrl}`)).arrayBuffer(),
          );

          assert.notDeepEqual(
            content1,
            content2,
            "Image file must be replaced when a new external URL is set on an existing herd entry",
          );
          assert.ok(
            entry2.imageUrl?.startsWith("/images/"),
            `Herd grid entry imageUrl should be local, got: ${entry2.imageUrl}`,
          );
        } finally {
          await deleteBreed(target.id);
          await deleteBreed(seedB.id);
    ...
    await t.test(
      "PUT myherd with non-external imageUrl is passed through unchanged",
      async () => {
        const { ck: userCk, id: userId } = await registerAndLoginUser(
          `testuser_${Date.now()}@test.invalid`,
        );
        try {
          ...
          await fetch(`${baseUrl}/api/myherd`, {
            ...
            headers: { "Content-Type": "application/json", Cookie: userCk },
            body: JSON.stringify([
              { id: 99902, name: "LocalBreed", imageUrl: localPath },
            ]),
          });
          ...
          assert.equal(
            entry.imageUrl,
            localPath,
            "Local /images/ paths should pass through unchanged",
          );
        } finally {
          await deleteAccount(userId);
        }
    ...
    // ── Teardown: delete the test admin account ────────────────────────────────
    {
      ...
      assert.equal(
        delRes.status,
        200,
        `Failed to delete test admin account: ${delRes.status}`,
      );
    }
  },
);
