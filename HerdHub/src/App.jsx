...
import ImageFilledIcon from '@mui/icons-material/Image';
...
export default function App() {
  ...
  // ── Session-expiry handler: called whenever an authenticated request gets 401 ─
  const handleSessionExpired = useCallback(() => {
    if (role === 'guest' || role === 'loading') return; // already logged out
  ...
  // ── Detect session expiry on any authenticated response ────────────────────
  useEffect(() => {
    if (role === 'loading' || role === 'guest') return;
    const origFetch = window.fetch;
    const intercepted = async (...args) => {
      const res = await origFetch(...args);
      if (res.status === 401) {
        const path = typeof args[0] === 'string' ? args[0] : args[0]?.url ?? '';
        if (path.startsWith('/api/')) handleSessionExpired();
      }
      return res;
    };
    window.fetch = intercepted;
    return () => { window.fetch = origFetch; };
  }, [role, handleSessionExpired]);
  ...
  const saveBreed = async (updated, originalName) => {
    if (editContext === 'master') {
      // Admin: patch master list. Server downloads external URLs, generates thumb synchronously,
      // and returns the final local path — use its response to update state.
      const resp = await fetch(`/api/breeds/${updated.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(updated),
      });
      const saved = resp.ok ? await resp.json() : updated;
      // Apply cache-buster so the browser bypasses stale cached image/thumb for this render.
      const withBust = saved.imageUrl
        ? { ...saved, imageUrl: `${saved.imageUrl}?t=${Date.now()}` }
        : saved;
      setBreeds((prev) => prev.map((b) => b.id === updated.id ? withBust : b));
      ...
      // Always re-fetch: server rewrites external URLs to local paths and generates thumbs
      // synchronously, so the fresh response contains the final state.
      const fresh = await fetch('/api/myherd').then((r) => r.json());
      // Apply cache-busters only to imageUrls that changed so browsers reload those images.
      const withBust = fresh.map((b) => {
        const prev = myHerd.find((p) => p.id === b.id);
        return b.imageUrl && prev?.imageUrl !== b.imageUrl
          ? { ...b, imageUrl: `${b.imageUrl}?t=${Date.now()}` }
          : b;
      });
      setMyHerd(withBust);
    }
  ...
  const filtered = useMemo(() => {
    ...
    return base.filter((b) => {
      ...
      const matchesImage = imageFilter === 'all' || (imageFilter === 'with' ? !!b.imageUrl : !b.imageUrl);
      return matchesSearch && matchesTag && matchesImage;
    });
  }, [breeds, myHerd, search, tab, tagFilter, imageFilter]);
      ...
      <Box sx={{ minHeight: '100vh', bgcolor: 'background.default' }}>
        ...
        <AppBar position="sticky" sx={{ bgcolor: '#1b4332', borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
          <Toolbar sx={{ gap: 1, flexWrap: 'wrap', py: { xs: 0.5, sm: 0 } }}>
            ...
            <Tabs
              value={tab}
              onChange={(_, v) => { if (v === 1 && !isUser) { setShowLogin(true); return; } setTab(v); setTagFilter(null); setImageFilter('all'); }}
              textColor="inherit"
              ...
              InputProps={{
                ...
                endAdornment: (
                  <InputAdornment position="end">
                    <Tooltip title={
                      imageFilter === 'all' ? 'Image filter: off' :
                      imageFilter === 'with' ? 'Image filter: has photo' :
                      'Image filter: no photo'
                    } arrow>
                      <IconButton
                        size="small"
                        onClick={() => setImageFilter((f) => f === 'all' ? 'with' : f === 'with' ? 'without' : 'all')}
                        sx={{
                          color: imageFilter !== 'all' ? 'white' : 'rgba(255,255,255,0.4)',
                          transition: 'color 0.2s',
                          '&:hover': { color: 'white' },
                        }}
                      >
                        {imageFilter === 'with'
                          ? <ImageFilledIcon fontSize="small" />
                          : imageFilter === 'without'
        ...
        <Snackbar
          open={sessionExpiredMsg}
          onClose={() => { setSessionExpiredMsg(false); setShowLogin(true); }}
