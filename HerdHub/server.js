...
function assertNoSsrf(rawUrl) {
  ...
  let hostname;
  try { hostname = new URL(rawUrl).hostname; } catch { throw new Error('Invalid URL'); }
  const h = hostname.toLowerCase().replace(/\.$/, '');
  if (h === 'localhost' || h === '0.0.0.0') throw new Error('SSRF: private host blocked');
...
function sanitizeUrl(url) {
  if (!url) return url;
  try { const { protocol } = new URL(url); if (protocol === 'http:' || protocol === 'https:') return url; } catch {}
  return null;
}
...
app.post('/api/login', authLimiter, async (req, res) => {
  ...
  if (remember) req.session.cookie.maxAge = 90 * 24 * 60 * 60 * 1000; // 90 days for "remember me"
...
app.patch('/api/breeds/:id', requireAdmin, async (req, res) => {
  try {
    ...
    // Reject non-http/https wikiUrls (e.g. javascript:) — A03
...
app.put('/api/myherd', requireUser, async (req, res) => {
  try {
    ...
    const clean = await Promise.all(herd.map(async (b) => {
      let imageUrl = b.imageUrl ? b.imageUrl.split('?')[0] : b.imageUrl;
      if (imageUrl && /^https?:\/\//.test(imageUrl)) {
        try {
          const nameSlug = (b.name || 'breed').replace(/[^a-zA-Z0-9]/g, '_').replace(/_+/g, '_').toLowerCase();
          const stem = b.id ? `${b.id}_${nameSlug}_1` : nameSlug;
          let ext;
          try { ext = path.extname(new URL(imageUrl).pathname).toLowerCase(); } catch { ext = ''; }
          ...
          await sharp(buf)
            ...
            .webp({ quality: 80 })
            .toFile(thumbDst)
            .catch((e) => console.warn(`Thumb gen failed for ${relpath}: ${e.message}`));
          imageUrl = `/images/${relpath}`;
        } catch (e) {
          console.warn(`Image download failed on PUT myherd: ${e.message}`);
        }
      }
      return { ...b, wikiUrl: b.wikiUrl ? sanitizeUrl(b.wikiUrl) : b.wikiUrl, imageUrl };
    }));
 ...
 */
async function downloadAndSaveImage(externalUrl, breedId, breedName, slot = 1, localImageHint = null, overwrite = true) {
  ...
  if (externalUrl.startsWith('data:')) {
    ...
    if (overwrite || !existsSync(dest)) {
      ...
      await generateThumb(filename).catch((e) => console.warn(`Thumb gen failed for ${filename}: ${e.message}`));
    }
...
async function ensureLocalImages() {
  try {
    ...
    for (const b of external) {
      ...
      if (idx === -1) continue;
      try {
        const localUrl = await downloadAndSaveImage(b.imageUrl, b.id, b.name, 1, b.localImage ?? null, false);
